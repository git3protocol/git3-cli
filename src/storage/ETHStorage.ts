            ...
            if (file.length > FileChunkSize) {
                ...
                for (uploadedSize = 0; uploadedSize < file.length; uploadedSize += FileChunkSize) {
                    ...
                    if (file.length != nouploadfileSize + uploadedSize){
                        throw new Error(`file.length${file.length} != nouploadfileSize${nouploadfileSize} + uploadedSize${uploadedSize}`)
                    }
                ...
                chunks.forEach(async (context,index) => {
                    console.error(`=== uploading file chunk ${path} chunkId-${index} chunk_size:${context.length} storage_cost:${costs[index]}-token===`)
